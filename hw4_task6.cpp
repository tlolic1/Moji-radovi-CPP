...
class Student {
    ...
    std::string OcistiString(const std::string &str) {
        ...
        rezultat.erase(novi_kraj, rezultat.end()); //rbegin i rend zad uz predavanje 7, unique uz 11, isspace spomenuto kada i isalpha i ostale slicne funkcije
        return rezultat;
    }
...
public:
    ...
    void RegistrirajNovogStudenta(int broj_indeksa, std::string godina_studija, std::string ime_prezime, std::string adresa, std::string broj_telefona) {
        ...
        studenti[broj_indeksa] = std::make_shared<Student>(broj_indeksa, godina_studija, ime_prezime, adresa, broj_telefona);
    }
